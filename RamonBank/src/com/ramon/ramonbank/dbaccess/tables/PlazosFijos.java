...
public class PlazosFijos extends Tables {
	...
	public String get_activo() {
		return _activo;
	}

	public void set_activo(String _activo) {
	...
	public String get_origen_nombre() {
		return ORIGEN.get_enum(this._origen).get_nombre();
	}

	public String get_acreditacion_nombre() {
		return PLAZO_FIJO.ACREDITACION.get_enum(this._acreditacion)
				.get_nombre();
	}
	...
	public PlazosFijos() {
		...
		this._acreditacion = -1;
		this._nroCuentaDestino = -1;
		this._monto = -1;
		this._interes = -1;
	...
	public ITables Load() throws OperationException {
		...
		try {
			if (rs.next()) {
				...
				oPlazosFijos.set_fechaAlta(rs.getDate("FechaAlta").toString());
				oPlazosFijos.set_fechaVencimiento(rs.getDate("FechaVencimiento")
						.toString());
				...
				oPlazosFijos.set_acreditacion(rs.getInt("Acreditacion"));
				oPlazosFijos
						.set_nroCuentaDestino(rs.getInt("NroCuentaDestino"));
				oPlazosFijos.set_monto(rs.getDouble("Monto"));
				oPlazosFijos.set_interes(rs.getDouble("Interes"));
				oPlazosFijos.set_activo(rs.getString("Activo"));
